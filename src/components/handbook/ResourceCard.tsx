...
export function ResourceCard({
      ...
      <div className="flex items-start justify-between gap-3">
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-xl transition-transform duration-200 group-hover:scale-110 ${
            isPdf ? "bg-orange-100 text-orange-500" : "bg-blue-100 text-blue-600"
          }`}
        >
        ...
        <span
          className={`rounded-full px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide ${
            isPdf ? "bg-orange-100 text-orange-600" : "bg-blue-100 text-blue-600"
          }`}
        >
          {isPdf ? "PDF" : "LINK"}
        </span>
  ...
  if (isLink && !card.view_link && !isDrivePreview(card.open_link)) {
    return (
